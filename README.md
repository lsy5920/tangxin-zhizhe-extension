...
这是 Chrome 扩展本地存储的正常保留机制。进入插件「工具」页，点击「清除数据缓存」，确认后会清除旧版本残留的 `chrome.storage.local` 数据、完整详情缓存、保存链路和页面 Hook 运行缓存。清理后刷新页面即可使用当前版本默认状态。

...
本文档描述扩展 5.0.3 的播放架构、状态边界、线路策略、持久化规则和验收标准。页面只负责展示统一 ViewModel，不直接持有 ArtPlayer、Hls、计时器或购买流程。
...
[2026-07-27 12:30] 扩展 5.0.3 把固定 `/vlog/` Swiper 的活动 Vue 项纳入统一页面上下文；首屏主动补检票、滑动按活动 ID 推进代次，并让手动刷新和下载复用同一当前视频解析器。

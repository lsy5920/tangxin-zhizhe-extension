...
| 已提交下载 | `submitted` | 显示后台下载编号或页面 Blob 交付方式，并明确仍需手动安装 | 到下载记录或扩展管理页完成安装 |
...
6. 校验成功后优先把同一份已验证内存字节编码为 `data:` 下载源，再调用后台 `chrome.downloads.download({ saveAs: true })`。如果 Kiwi、Android Chromium 或当前扩展后台没有该 API，或调用被浏览器拒绝，后台会把同一份字节通过扩展消息交给 `update_downloader.js`；页面先重新核对字节数、CRX3 文件头和 SHA-256，再创建临时 Blob 链接拉起保存。两条路径都不会重新访问远程镜像，因此没有“探测时是安全文件、实际下载时被替换”的竞态窗口。
...
- 浏览器返回下载 ID，或页面 Blob 已被点击提交后，即使状态持久化失败，也立即保留该成功结果，绝不切换镜像重复创建下载任务。
...
只有 `npm run release` 会要求源码版本、`update.json`、`latest.json` 和 CRX 完全一致，并使用固定私钥重新签名。当前源码与签名发布均为 `5.0.5 / 2026-07-27-1315`，继续使用 4.0.0 启用的固定私钥，扩展 ID 保持为 `ddefadnhgebdclpkabeobjidjllkdkhm`。
...
- `background.js`：签名清单验证、多源比较、防回退、完整 CRX3 验证、状态串行写入，以及后台下载/页面交付双通道编排。
- `update_downloader.js`：只在后台下载 API 不可用时接收已验证字节，重新复核大小、CRX3 文件头和 SHA-256，并通过当前页面 Blob 提交下载。
- `offscreen_downloader.js`：承载视频分片合并下载；CRX 更新不依赖离屏页，兼容缺少 `downloads` 的离屏实现。
- `content.js`：发布真实检测/下载阶段，设置消息超时，转发明确的忽略与下载动作，并在需要时完成页面 Blob 交付与结果确认。

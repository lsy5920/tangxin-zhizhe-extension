...
只有 `npm run release` 会要求源码版本、`update.json`、`latest.json` 和 CRX 完全一致，并使用固定私钥重新签名。当前目标源码为 `5.2.1 / 2026-07-29-1929`，继续使用 4.0.0 启用的固定私钥，扩展 ID 保持为 `ddefadnhgebdclpkabeobjidjllkdkhm`。
...
5. 使用全新 Chromium 持久化上下文真实加载扩展，确认服务线程、正式 ID、内容脚本、更新检测、OPFS 保存页、关闭后重试、多标签领取和关键界面。
...
[2026-07-29 19:29] 扩展 5.2.1 修复 Shadow DOM 下 Tailwind 注册属性失效造成的生产样式缺失；发布门禁新增属性基线、内联 CSS 和预览运行路径校验，正式身份与 CRX 保存协议不变。
